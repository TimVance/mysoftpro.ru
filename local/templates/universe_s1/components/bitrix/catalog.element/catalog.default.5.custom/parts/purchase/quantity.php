...
<?php $vQuantity = function ($arItem, $bOffer = false) use (&$arVisual) { ?>
    ...
    <?= Html::beginTag('div', [
        ...
        <?php if ($arItem['CAN_BUY']) { ?>
            ...
            <?php } ?>
        <?php } elseif($arItem['ACTION'] == 'order') { ?>
                <div class="catalog-element-quantity-indicator-container catalog-element-quantity-part">
					<div class="catalog-element-quantity-indicator-2">
						<span class="icon" data-toggle="tooltip" data-placement="top" title="Сроки поставки уточняйте"><i class="far fa-question-circle"></i></span>
					</div>
                </div>
                <div class="catalog-element-quantity-value-container catalog-element-quantity-part">
                    <?= Html::tag('span', Loc::getMessage('C_CATALOG_ELEMENT_DEFAULT_5_TEMPLATE_UNDER_ORDER'), [
                        'class' => 'catalog-element-quantity-value-text',
                        'data' => [
                            'quantity-state' => 'many',
                            'store-use' => $arVisual['STORES']['USE'] && $arVisual['STORES']['POSITION'] === 'popup' ? 'true' : 'false'
